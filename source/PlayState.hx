...
class PlayState extends MusicBeatState {
	...
	override public function create() {
		...
		switch (songLowercase) {
			...
			case 'coffe':
				dialogue = CoolUtil.coolTextFile(Paths.txt('coffe/dialogue-kyle-coffe'));
			case 'work':
				dialogue = CoolUtil.coolTextFile(Paths.txt('work/Dialogue-kyle-work'));
			case 'tired':
				dialogue = CoolUtil.coolTextFile(Paths.txt('tired/dialogue-kyle-tired'));
		}
		...
		if (!PlayStateChangeables.Optimize) {
			switch (stageCheck) {
				...
				case 'kylekitchen':
					{
						defaultCamZoom = 0.9;
						curStage = 'kylekitchen';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('kylekitchen/kylekitchen'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('kylekitchen/kylekitchenshading'));
	...
	private function generateStaticArrows(player:Int):Void {
		for (i in 0...4) {
			...
			switch (noteTypeCheck) {
				...
				case 'kyle-tired':
					babyArrow.frames = Paths.getSparrowAtlas('tiredarrows/NOTE_assets_tired');
